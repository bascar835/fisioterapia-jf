...
BASE = "https://TU-USUARIO.github.io/fisioterapia-jf/"
...
def generate_post_pages(posts):
    ...
    for post in posts:
        ...
        title_tag = f"{post['title']} | Fisioterapia y Osteopatía JF"
        description = post["excerpt"].split("\n")[0]
        image_url = f"{BASE}{post['image']}"
        keywords = f"{post['category'].lower()}, fisioterapia Benidorm, Fisioterapia y Osteopatía JF"

        doc = template
        doc = doc.replace("<title>Artículo | Fisioterapia y Osteopatía JF</title>", f"<title>{title_tag}</title>")
        ...
        doc = doc.replace('<meta property="og:title" content="Artículo | Fisioterapia y Osteopatía JF">',
        ...
        doc = doc.replace('<meta name="twitter:title" content="Artículo | Fisioterapia y Osteopatía JF">',
        ...
        doc = doc.replace('"name": "Fisioterapia y Osteopatía JF",\r\n    "url": "post.html",\r\n    "potentialAction"',
                           f'"name": "Fisioterapia y Osteopatía JF",\r\n    "url": "{url}",\r\n    "potentialAction"')
        ...
        schema = {
            ...
            "@type": "BlogPosting",
            "headline": post["title"],
            "author": {"@type": "Organization", "name": post.get("author", "Fisioterapia y Osteopatía JF")},
            "datePublished": post["date"],
            "dateModified": post["date"],
            "image": [image_url],
            "description": description,
            "mainEntityOfPage": url,
            "publisher": {"@type": "Organization", "name": "Fisioterapia y Osteopatía JF",
        ...
        doc = doc.replace(
            ...
            f'<div class="post-meta" id="post-meta"><span>{post["date"]}</span><span>{post.get("author", "Fisioterapia y Osteopatía JF")}</span><span>{post["category"]}</span></div>'
        )
        doc = doc.replace(
        ...
        doc = doc.replace(
            '<strong id="post-author">Fisioterapia y Osteopatía JF</strong>',
            f'<strong id="post-author">{post.get("author", "Fisioterapia y Osteopatía JF")}</strong>'
        )
        doc = doc.replace(
...
if __name__ == "__main__":
    posts = load_posts()
    menu = load_json("services.json", {"categories": []})
