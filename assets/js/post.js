...
function renderShareButtons(post) {
  ...
  const text = `${post.title} - Fisioterapia y Osteopatía JF`;
...
function updateStructuredData(post) {
  const schema = {
    ...
    publisher: { '@type': 'Organization', name: 'Fisioterapia y Osteopatía JF', logo: { '@type': 'ImageObject', url: 'assets/img/logo.png' } },
  };
...
function applyPostMeta(post) {
  document.title = `${post.title} | Fisioterapia y Osteopatía JF`;
  setMetaTag('meta[name="description"]', 'content', post.excerpt);
  setMetaTag('meta[name="keywords"]', 'content', `${post.category.toLowerCase()}, fisioterapia, Benidorm, Fisioterapia y Osteopatía JF`);
