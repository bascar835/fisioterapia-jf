const RF_SITE = {
  name: 'Fisioterapia y Osteopatía JF',
  url: './',
  phone: '+34 608 50 59 83',
  email: 'info@fisioterapiajf.es',
  address: 'C. de las Escuelas, nº3, Benidorm, Alicante',
  mapQuery: 'Fisioterapia y Osteopatía JF Benidorm',
};

const RF_DATA = {
  featuredLinks: [
    { label: 'Sobre la clínica', anchor: '#nosotros' },
...
async function loadSiteData() {
  const [postsData, menuData, galleryData, homeData] = await Promise.all([
    ...
    fetchJson('services.json', { categories: [] }),
...
function renderHeroMedia() {
  ...
  container.innerHTML = `<img src="${media.image || 'assets/img/hero.jpg'}" alt="Fachada y ambiente de Fisioterapia y Osteopatía JF en Benidorm" class="hero-image" data-parallax>`;
}
...
function updateBlogPostingSchema() {
  ...
  const schema = {
    ...
    '@type': 'BlogPosting',
    headline: post.title,
    author: { '@type': 'Organization', name: 'Fisioterapia y Osteopatía JF' },
